...
{
public:
	vertex()					: m_Pnt(0.0F, 0.0F)			{m_index = 0;}
	vertex(const vertex& v, size_t index)		: m_Pnt(v.m_Pnt)			{m_index = index;}
	vertex(const PointF& pnt, size_t index)	: m_Pnt(pnt)				{m_index = index;}
	vertex(REAL x, REAL y, size_t index)		: m_Pnt(x, y){m_index = index;}
	//vertex(int x, int y, size_t index)		: m_Pnt((REAL) x, (REAL) y)	{m_index = new size_t; *m_index = index;}
...
public:
	...
	double getLength(){
		return sqrt((m_pV0->GetX() - m_pV1->GetX()) * (m_pV0->GetX() - m_pV1->GetX()) + (m_pV0->GetY() - m_pV1->GetY()) * (m_pV0->GetY() - m_pV1->GetY()));
	}
};
