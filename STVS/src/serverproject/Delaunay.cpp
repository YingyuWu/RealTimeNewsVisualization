...
void Delaunay::Triangulate(const vertexSet& vertices, triangleSet& output)
{
	...
	vSuper[0] = vertex(xMin - dy * sqrt3 / 3.0F, yMin, 1000);	// Simple highschool geometry, believe me.
	vSuper[1] = vertex(xMax + dy * sqrt3 / 3.0F, yMin, 1000);
	vSuper[2] = vertex((xMin + xMax) * 0.5F, yMax + dx * sqrt3 * 0.5F, 1000);

	triangleSet workset;
	workset.insert(triangle(vSuper));
	
	// TODO : Fix the buf for VS 2012
	/*
	for (itVertex = vertices.begin(), iterator_index = 0; itVertex != vertices.end(); itVertex++, iterator_index++)
	{
		...
		tIterator itEnd = std::remove_if(workset.begin(), workset.end(), triangleIsCompleted(itVertex, output, vSuper));
	...
	*/
	...
	for (itVertex = vertices.begin(), iterator_index = 0; itVertex != vertices.end(); itVertex++, iterator_index++)
	{
		...
		itEnd = workset.end();
		tIterator first = workset.begin();
		tIterator last = itEnd;
		tIterator result = first;
		//while (first != last) {
		//	if (triangleIsCompleted(itVertex, output, vSuper)(*first)) {
		//		//(triangle)*result = std::move(*first);
		//		result = first;
		//		if (++first == last) {
		//			workset.erase(result);
		//			break;
		//		}
		//		else {
		//			workset.erase(result);
		...
		*/
		...
		while (first!=last) {
			if (vertexIsInCircumCircle(itVertex, edges)(*first)) {
				//(triangle)*result = std::move(*first);
				result = first;
				if (++first == last) {
					workset.erase(result);
	...
	// END TODO : Fix the buf for VS 2012
